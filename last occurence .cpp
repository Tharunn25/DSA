// cpp program to find the last occurence of the element in a array containing duplicate elements
#include <iostream>
#include <bits/stdc++.h>
using namespace std;
...
int search(int arr[], int n, int x)
{
    ...
    while (low <= high)
    {
        ...
        if (arr[mid] == x)
        {
            ...
            low = mid + 1; // since the last occurence can't be before the mid so we r only searching in
            continue;       // right side of the element so low = mid+1
        }
        if (arr[mid] > x)
            high = mid - 1;
        if (arr[mid] < x)
